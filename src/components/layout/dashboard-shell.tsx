...
export function DashboardShell({ children, profile, activeEvent }: { children: React.ReactNode; profile: Profile | null; activeEvent?: ActiveEvent | null }) {
  const [collapsed, setCollapsed] = useState(false)

  return (
    <div className="min-h-screen flex flex-col bg-[linear-gradient(var(--aroos-bg-from)_0%,var(--aroos-bg-to)_100%)] bg-fixed">
      <TopNav collapsed={collapsed} profile={profile} activeEvent={activeEvent} />
