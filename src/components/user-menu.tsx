      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-50 w-[390px] rounded-2xl border border-[#1B1B1B]/08 bg-white shadow-[0_12px_40px_rgba(27,27,27,0.16)] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="px-5 py-4 border-b border-[#1B1B1B]/06">
            <p className="text-[15px] font-semibold text-[#1B1B1B]">{fullName}</p>
            <p className="text-[13px] text-[#1B1B1B]/50 mt-0.5">{email}</p>
            <span className="mt-2 inline-block rounded-full bg-aroos-accent/10 px-2.5 py-0.5 text-[12px] font-medium text-aroos-accent">
              {role}
            </span>
          </div>
          ...
          <div className="p-2">
            <a href="/settings"
              className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] text-[#1B1B1B] hover:bg-[#1B1B1B]/[0.05] transition-colors"
              onClick={() => setOpen(false)}
            >
              <Settings className="size-4 text-[#1B1B1B]/50 shrink-0" />
              Profile settings
            </a>
            <a href="/events"
              className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] text-[#1B1B1B] hover:bg-[#1B1B1B]/[0.05] transition-colors"
              onClick={() => setOpen(false)}
            >
              <User className="size-4 text-[#1B1B1B]/50 shrink-0" />
              Switch event
            </a>
          </div>
          ...
          <div className="p-2 border-t border-[#1B1B1B]/06">
            <form action={signOut}>
              <button type="submit"
                className="w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] text-red-500 hover:bg-red-50 transition-colors text-left"
              >
